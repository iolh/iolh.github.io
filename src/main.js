...
// 创建两个背景音乐实例
const dayBackgroundMusic = new Howl({
  src: ["/audio/music/yuan_you_hui.mp3"],
  loop: true,
  volume: 1,
});

const nightBackgroundMusic = new Howl({
  src: ["/audio/music/happy_birthday.mp3"],
  loop: true,
  volume: 1,
});

// 当前播放的背景音乐
let currentBackgroundMusic = dayBackgroundMusic;

// 切换背景音乐的函数
const switchBackgroundMusic = (isNight) => {
  const wasPlaying = currentBackgroundMusic.playing();
  const currentVolume = currentBackgroundMusic.volume();
  
  // 停止当前音乐
  currentBackgroundMusic.stop();
  
  // 切换到新的音乐
  currentBackgroundMusic = isNight ? nightBackgroundMusic : dayBackgroundMusic;
  
  // 如果之前正在播放，则开始播放新音乐
  if (wasPlaying) {
    currentBackgroundMusic.volume(currentVolume);
...
const fadeOutBackgroundMusic = () => {
  if (!isMuted && !isMusicFaded) {
    currentBackgroundMusic.fade(
      currentBackgroundMusic.volume(),
      FADED_VOLUME,
      MUSIC_FADE_TIME
    );
    isMusicFaded = true;
...
manager.onLoad = function () {
  ...
  noSoundButton.textContent = "悄悄进入 :(";
...
const updateMuteState = (muted) => {
  if (muted) {
    ...
    currentBackgroundMusic.volume(BACKGROUND_MUSIC_VOLUME);
  }
...
const handleMuteToggle = (e) => {
  ...
  if (!currentBackgroundMusic.playing()) {
    currentBackgroundMusic.play();
  }
